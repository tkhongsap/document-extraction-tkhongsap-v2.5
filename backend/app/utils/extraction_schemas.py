...
LINE_ITEMS_KEYS: Dict[DocumentType, str | None] = {
    "bank": "transactions",
    "invoice": "line_items",
    "po": "line_items",
    "contract": "parties",
...
def get_schema_for_type(document_type: DocumentType) -> Dict[str, Any]:
...
def get_document_type_name(document_type: DocumentType) -> str:
    """Get human-readable name for document type"""
    return DOCUMENT_TYPE_NAMES.get(document_type, document_type)


def get_line_items_key(document_type: DocumentType) -> str | None:
    """Get the key for line items based on document type"""
    return LINE_ITEMS_KEYS.get(document_type)
